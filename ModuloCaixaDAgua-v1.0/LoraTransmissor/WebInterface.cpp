...
<div class="config-section" id="configSection">
  ...
  <div class="card">
    ...
    <small>⚠️ Bandwidth deve ser IGUAL no transmissor e receptor!</small>
  </div>
...
async function loadConfig() {
  try {
    ...
    document.getElementById('loraBandwidth').value = data.lora_bandwidth || 125000;  // ← ADICIONAR
...
async function saveConfig() {
  const config = {
    ...
    lora_bandwidth: parseInt(document.getElementById('loraBandwidth').value),  // ← ADICIONAR
...
void WebInterface::handleApiConfig() {
    ...
    doc["lora_bandwidth"] = lora->getBandwidth();  // ← ADICIONAR
