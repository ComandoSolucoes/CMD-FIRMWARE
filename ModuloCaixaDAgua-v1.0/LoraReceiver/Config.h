#ifndef CONFIG_LORA_RECEIVER_H
#define CONFIG_LORA_RECEIVER_H

#include <Arduino.h>

// ==================== IDENTIFICAÇÃO ====================
#define DEVICE_MODEL "LORA-RECEIVER"
#define FIRMWARE_VERSION "1.0.0"

// ==================== PINAGENS ====================
// NeoPixel
#define PIN_LED        2
#define NUMPIXELS      1
#define LED_BRIGHTNESS 60

// LoRa (SX1276/SX1278)
#define LORA_SCK   18
#define LORA_MISO  19
#define LORA_MOSI  23
#define LORA_SS    5
#define LORA_RST   14
#define LORA_DIO0  26

// ==================== CONFIGURAÇÕES LORA ====================
// ⚠️ ATENÇÃO: DEVEM SER IDÊNTICOS NO TRANSMISSOR E RECEPTOR!

#define LORA_FREQUENCY         915E6    // 915 MHz (América do Sul)
                                        // 433E6 para Ásia
                                        // 868E6 para Europa

// Parâmetros de Modulação (IDÊNTICOS AO TRANSMISSOR!)
#define DEFAULT_LORA_SPREADING       7        // SF7 (6-12)
#define DEFAULT_LORA_BANDWIDTH       125E3    // 125 kHz
#define DEFAULT_LORA_CODING_RATE     5        // 4/5 (5-8)
#define DEFAULT_LORA_SYNC_WORD       0x12     // 0x12 = rede privada

// Limites
#define MIN_LORA_SPREADING       6
#define MAX_LORA_SPREADING       12

// ==================== MQTT ====================
#define MQTT_TOPIC_RECEIVED      "lora/received"

// ==================== LOG DE MENSAGENS ====================
#define MAX_LOG_MESSAGES         5

// ==================== LED FEEDBACK ====================
#define LED_COLOR_OFF         0,   0,   0
#define LED_COLOR_IDLE        0,   200, 0
#define LED_COLOR_RECEIVING   0,   0,   255
#define LED_COLOR_ERROR       255, 0,   0
#define LED_COLOR_WIFI_AP     255, 255, 0

#define LED_BLINK_ON_MS       80
#define LED_BLINK_OFF_MS      60

// ==================== PREFERENCES ====================
#define PREFS_NAMESPACE           "lora-receiver"
#define PREFS_LORA_SPREADING      "lora_sf"
#define PREFS_LORA_BANDWIDTH      "lora_bw"
#define PREFS_LORA_CODING_RATE    "lora_cr"
#define PREFS_LORA_SYNC_WORD      "lora_sw"
#define PREFS_AUTO_PUBLISH        "auto_pub"

// ==================== ESTRUTURAS ====================
struct LoRaConfig {
    uint8_t spreadingFactor;
    uint8_t codingRate;
    uint32_t bandwidth;
    uint8_t syncWord;
};

struct ReceivedMessage {
    String payload;
    int rssi;
    float snr;
    unsigned long timestamp;
};

// ==================== LOGS ====================
#define LOG_PREFIX "[LORA-RX] "

#define LOG_INFO(msg)       Serial.println(LOG_PREFIX msg)
#define LOG_INFOF(fmt, ...) Serial.printf(LOG_PREFIX fmt "\n", ##__VA_ARGS__)
#define LOG_ERROR(msg)      Serial.println(LOG_PREFIX "❌ " msg)
#define LOG_ERRORF(fmt, ...) Serial.printf(LOG_PREFIX "❌ " fmt "\n", ##__VA_ARGS__)

#endif