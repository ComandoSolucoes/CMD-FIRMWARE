...
void WebInterface::registerRoutes(CMDWebServer* webServer) {
    ...
    // Atribui ponteiro ao servidor
    server = &webServer->getServer();
    
    // APIs REST
    server->on("/api/device/status", HTTP_GET, [this]() {
        JsonDocument doc;
    ...
    server->on("/api/device/config", HTTP_GET, [this]() {
        JsonDocument doc;
        
        doc["relay_logic"] = (uint8_t)io->getRelayLogic();
        doc["initial_state"] = (uint8_t)io->getInitialState();
        doc["publish_interval"] = mqtt->getPublishInterval();
        
        JsonArray inputs = doc["inputs"].to<JsonArray>();
        for (uint8_t i = 0; i < NUM_INPUTS; i++) {
            JsonObject input = inputs.add<JsonObject>();
            input["mode"] = (uint8_t)io->getInputMode(i);
            input["debounce"] = io->getDebounceTime(i);
            input["pulse"] = io->getPulseTime(i);
        }
    ...
    server->on("/api/device/config", HTTP_POST, [this]() {
        ...
        if (doc.containsKey("relay_logic")) {
            io->setRelayLogic((RelayLogic)doc["relay_logic"].as<uint8_t>());
        }
        
        if (doc.containsKey("initial_state")) {
            io->setInitialState((InitialState)doc["initial_state"].as<uint8_t>());
        }
        ...
        if (doc.containsKey("inputs")) {
            ...
            for (uint8_t i = 0; i < NUM_INPUTS && i < inputs.size(); i++) {
                ...
                if (input.containsKey("mode")) {
                    io->setInputMode(i, (InputMode)input["mode"].as<uint8_t>());
                }
                if (input.containsKey("debounce")) {
                    io->setDebounceTime(i, input["debounce"].as<uint16_t>());
                }
                if (input.containsKey("pulse")) {
                    io->setPulseTime(i, input["pulse"].as<uint16_t>());
                }
            }
        }
    ...
    server->on("/api/device/relay/all", HTTP_POST, [this]() {
        bool allOn = true;
        for (uint8_t i = 0; i < NUM_OUTPUTS; i++) {
            if (!io->getRelayState(i)) {
                allOn = false;
                break;
            }
        }
        
        io->setAllRelays(!allOn);
        server->send(200, "application/json", "{\"success\":true}");
    });
    
    // Rotas dinâmicas de relés (0-7)
    for (uint8_t i = 0; i < NUM_OUTPUTS; i++) {
        String path = "/api/device/relay/" + String(i);
        server->on(path.c_str(), HTTP_POST, [this, i]() {
            io->toggleRelay(i);
            server->send(200, "application/json", "{\"success\":true}");
        });
    }
    
    LOG_INFO("Rotas customizadas registradas (8 canais)");
}
...
<title>8 Canais I/O - CMD-C</title>
<style>
...
.relay-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin:20px 0}
.relay-btn{padding:16px;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;transition:all 0.3s;display:flex;flex-direction:column;align-items:center;gap:6px}
...
.relay-icon{font-size:28px}
.input-status{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin:15px 0}
.input-box{padding:10px;border-radius:8px;text-align:center;font-weight:600;font-size:13px}
...
.input-config-grid{display:grid;gap:15px}
.input-config-card{background:#f8f9ff;padding:15px;border-radius:8px;border-left:4px solid #667eea}
.input-config-card h3{color:#667eea;margin-bottom:10px;font-size:16px}
@media(max-width:600px){.form-row{grid-template-columns:1fr}.relay-grid{grid-template-columns:repeat(2,1fr)}}
</style>
</head><body>
<div class="container">
<h1>🎛️ 8 Canais I/O</h1>
<div class="subtitle">8 Entradas + 8 Saídas</div>
...
<div class="config-section" id="configSection">
  ...
  <div class="card">
    ...
    <div class="input-config-grid" id="inputConfigs"></div>
  </div>
...
function updateRelayGrid(relays) {
  ...
  for (let i = 0; i < relays.length; i++) {
    ...
    btn.innerHTML = `
      ...
      <div>Relé ${i + 1}</div>
      <div style="font-size:11px">${state ? 'ON' : 'OFF'}</div>
    `;
...
function updateInputStatus(inputs) {
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    box.innerHTML = `
      <div>IN ${i + 1}</div>
      <div style="font-size:16px;margin-top:4px">${state ? '⬆️ HIGH' : '⬇️ LOW'}</div>
    `;
